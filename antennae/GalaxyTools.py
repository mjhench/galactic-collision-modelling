...
class GalaxyTools:
    """
    Various useful utilities for simulating the antennae galaxies.

    Methods:
        spherical_to_xyz(r, theta, phi)                : convert spherical to Cartesian coordinates
        cylindrical_to_xyz(r, phi, z)                  : convert cylindrical to Cartesian coordinates
        rotate(x_all, y_all, z_all, axis, theta)       : rotate position vectors about a given axis
        vel_init(G, M, theta, r_ring, e)               : calculate Kepler initial velocity given radius and angle
        gen_sim(path, num_part, num_steps, view_angle) : generate the plots for a desired view angle for ffmpeg
        read_data(path, num_part, num_steps)           : read the data file generated by leapfrog integration
        make_video(out_name)                           : make a video file from the plots stored in ./plots using ffmpeg
    """
    ...
    def gen_sim(path, num_part, num_steps, view_angle):
        ...
        data = GalaxyTools.read_data(path, num_part, num_steps)
        ...
        with Pool(8) as p:
            p.starmap(GalaxyTools.work, args)
    ...
    def make_video(out_name):
        ...
        cmd_str = f"ffmpeg -y -r 24 -f image2 -s 640x480 -i plots/fig_%d.png -vcodec libx264 -crf 25 -pix_fmt yuv420p anim/{out_name}.mp4"
        os.system(cmd_str)

    @staticmethod
    def work(t, data, ax, view_angle):
        """
